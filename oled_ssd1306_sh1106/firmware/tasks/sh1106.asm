...
#define GRAPHICS_BUFFER_SIZE (SH1106_WIDTH*(SH1106_HEIGHT/8)) 
.dseg
graphics_buffer:   .byte GRAPHICS_BUFFER_SIZE
.cseg

.EQU	SH1106_ADDRESS =0x3C   ;SH1106
...
sh1106_setup:
	...
	ldi axl,DISPLAY_OFF		
	rcall sh1106_send_command

    ldi axl,SET_MEMORY_ADDRESSING_MODE		
	rcall sh1106_send_command
	
    ldi axl,2		;PAGE
	rcall sh1106_send_command
    
    ldi axl,SET_PAGE_ADDRESS		
	rcall sh1106_send_command

	ldi axl,SET_COM_OUTPUT_SCAN_DIRECTION		
	rcall sh1106_send_command

	ldi axl,LOW_COLUMN_ADDRESS		
	rcall sh1106_send_command

	ldi axl,HIGH_COLUMN_ADDRESS		
	rcall sh1106_send_command

	ldi axl,START_LINE_ADDRESS		
	rcall sh1106_send_command
    
	ldi axl,SET_CONTRAST_CTRL_REG		
	rcall sh1106_send_command
    
	ldi axl,0x7F		
	rcall sh1106_send_command

	ldi axl,SET_SEGMENT_REMAP		
	rcall sh1106_send_command

	ldi axl,SET_NORMAL_DISPLAY		
	rcall sh1106_send_command
 
 	ldi axl,SET_MULTIPLEX_RATIO		
	rcall sh1106_send_command

 	ldi axl,0x3F		
	rcall sh1106_send_command

	ldi axl,OUTPUT_FOLLOWS_RAM		
	rcall sh1106_send_command

	ldi axl,SET_DISPLAY_OFFSET		
	rcall sh1106_send_command

	ldi axl,0x00		
	rcall sh1106_send_command
 
	ldi axl,SET_DISPLAY_CLOCK_DIVIDE		
	rcall sh1106_send_command
    
	ldi axl,0xF0		
	rcall sh1106_send_command

	ldi axl,SET_PRE_CHARGE_PERIOD		
	rcall sh1106_send_command

  	ldi axl,0x22		
	rcall sh1106_send_command
 
  	ldi axl,SET_COM_PINS_HARDWARE_CONFIG		
	rcall sh1106_send_command
...
;@USED: axl
******************************************************************************************/
sh1106_horz_scroll_right:
    ldi axl,SCROLL_HORZ_LEFT		
	rcall sh1106_send_command
...
shclr_loop_11:
    ...
    ;data continue
	ldi argument,SH1106_DATA_CONTINUE
	rcall twi_send_byte
	;expect status ACK	
	lds temp,TWSR
	andi temp, 0xF8
	...
	;send 16x8=OLED_WIDTH
	
	ldi temp,SH1106_COLS
	mov r15,temp
...
shclr_loop_00:		
	...
	;transmit end condition
	;rcall twi_send_stop			;deviate from C code

	inc axh
	cpi axh,SH1106_PAGES
	brne shclr_loop_11 
...
buf_8bit_roboto_loop:			; send bits one by one	
   ...
   clr temp
   mov char,temp
   rcall sh1106_draw_buffer_pixel   ;input=X,Y,char   							
   rjmp black_end_roboto_00

black_out_roboto_00:			//BLACK pixel
   ser temp
   mov char,temp
   call sh1106_draw_buffer_pixel	;input=X,Y,char
