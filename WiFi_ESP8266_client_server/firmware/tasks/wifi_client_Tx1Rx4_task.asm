;***********************************************USART 0***********************************************
;WiFi communication in client mode
;Transmit 1 byte ; Recieve 4 bytes
;*****************************************************************************************************
...
recv_buffer: .byte 4 ; expecting 4 bytes integer value
...
wifi_main:
//send 1 byte recieve 4 bytes as integer value
wfmn_00:
...
wfmn_02:  //resend byte again
    ...
    ldi argument,'?'	;mark begin communiction with wifi
    rcall send_usart_queue 
	rcall send_tcp_data ;1 byte send 
	
	;receive 4 bytes Collect bytes in r12,r11,r10,r9
	rcall recv_tcp_data
...
; The server should respond with 4 bytes
; ESP-01 will automatically forward TCP data to UART
; in the format: +IPD,4:xxxx
;@USAGE: temp,cxl,Z,axl,return,XL,XH,bxl,bxh,r0,r1,r17,YL,YH
;=========================================================
recv_tcp_data:
rcv_00:    
    ldi temp, 200        ; ~6 second timeout (adjust as needed)
...
rcv_06:
	;read 4 bytes integer value 
	ldi temp, 4
	mov cxl,temp
	;into buffer
	ldi XL,low(recv_buffer)
	ldi XH,high(recv_buffer)
rcv_06_00:
    ldi ZL,low(wifi_queue)
    ldi ZH,high(wifi_queue)	  	
    ldi axl,WIFI_QUEUE_MAX_SIZE		
