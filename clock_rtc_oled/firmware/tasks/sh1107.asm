...
#define DEJAVU_CHAR_COLS_LEN        16                 // number of columns for a chars (bits)
#define DEJAVU_CHARS_ROWS_LEN        3*8                 // number of rows for chars   (bits)
#define DEJAVU_CHAR_SIZE        48         //48 bytes according to the table

/******************************************************************************************
;Send single DEJAVU font char to buffer {only digits and special chars , NO ALPHABET chars!}
;Position 0=<X=<127 and 0=<Y=<127 and draw pixel into buffer
;@INPUT  XX 
;		 YY
;		 argument - default font charecter to draw
...
;@USED:  temp,char,Z,r20,r21,r0,r1,r6,r7,r8,r9,r10,r11
******************************************************************************************/
sh1107_draw_buffer_char_dejavu:
    ;test if outside of drawing area
	mov temp,XX				;test XX
	subi temp,-1*DEJAVU_CHAR_COLS_LEN
	cpi temp,(SH1107_WIDTH)     
	brlo buf_char_dejavu_yy
ret
buf_char_dejavu_yy:        
	mov temp,YY				;test YY
	subi temp,-1*DEJAVU_CHARS_ROWS_LEN
	cpi temp,(SH1107_HEIGHT)     
	brlo buf_char_dejavu_ok
ret
buf_char_dejavu_ok:
   subi argument,32
   
   ;translate to bytes representation in fonts table
   ldi	ZH,high(deja_vu_sans_mono_bold_16x24*2)
   ldi	ZL,low(deja_vu_sans_mono_bold_16x24*2)

   ldi r20,DEJAVU_CHAR_SIZE	   //cols const in table each char is represented by 24 bytes
   mov r21,argument             //row number variable
   mul r20,r21

   ADD16 ZL,ZH,r0,r1			//offset to char row in table

   ldi temp, 3      //font roboto has 3 rows by 8 bits each or 32 bits height
   mov r6,temp

buf_next_half_char_dejavu_00:
   tst r6				//are 3 halfs by 8 bits done?
   breq buf_next_half_char_dejavu_end

   ldi temp,DEJAVU_CHAR_COLS_LEN   //loop throu 16 columns 
   mov r10,temp              //r10 counter 
...
buf_char_dejavu_00: 
   tst r10
   breq buf_next_half_char_dejavu_01   	

   lpm					;read next col from font 16x32
   mov	r11,r0	        ;r11 is char byte
...
buf_8bit_dejavu_loop:			; send bits one by one	
   tst r9
   breq buf_8bit_dejavu_end

   ror r11
   brcs	black_out_dejavu_00	
						//WHITE pixel
   clr temp
   mov char,temp
   rcall sh1107_draw_buffer_pixel   ;input=X,Y,char   							
   rjmp black_end_dejavu_00

black_out_dejavu_00:			//BLACK pixel
   ser temp
   mov char,temp
   call sh1107_draw_buffer_pixel	;input=X,Y,char
...
black_end_dejavu_00:
   ...
   dec r9		;go to next bit
   rjmp buf_8bit_dejavu_loop

buf_8bit_dejavu_end:
   adiw ZH:ZL,1         //move to next column
   dec r10
   inc XX
   rjmp buf_char_dejavu_00

buf_next_half_char_dejavu_01:
  dec r6    //next font half
  mov XX,r7			//reset X
  rjmp buf_next_half_char_dejavu_00

buf_next_half_char_dejavu_end:
